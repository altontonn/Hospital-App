                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="appointment_list_table">
                                    ...
                                    <tr>
                                        ...
                                        <td><button type="button" class="btn btn-primary text-white email-button" name="email-button" id="<?php echo $count ?>" data-user="<?php echo $row1["Firstname"] ?> <?php echo $row1["Lastname"] ?>" data-physiotheraphy="<?php echo $row["User_id"] ?>" data-contact="<?php echo $row1["Phone"] ?>" data-date="<?php echo $row["schedule_date"] ?>" data-day="<?php echo $row["schedule_day"] ?>" data-start="<?php echo $row["start_time"] ?>" data-end="<?php echo $row["end_time"] ?>">Book Appointment</button></td>
                                        </tr>
                                        <?php
                                    }
                                    ?>
...
<script>
    $(document).ready(function(){
        $('.email-button').click(function(){
            ...
            email_data.push({
                user: $(this).data("user"),
                physiotheraphy: $(this).data("physiotheraphy"),
