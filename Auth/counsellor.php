...
<?php
    ...
    if(isset($_POST['fname']) && isset($_POST['lname']) && isset($_POST['sname']) && isset($_POST['email']) && isset($_POST['pwd']) && isset($_POST['dob']) && isset($_POST['gender']) && isset($_POST['status']) && isset($_POST['phone']) && isset($_POST['address'])){
        ...
        if(!empty($fname) && !empty($lname) && !empty($sname) && !empty($email) && !empty($pwd) && !empty($dob) && !empty($gender) && !empty($status) && !empty($phone) && !empty($address)){
            $query = "SELECT `Firstname`, `Lastname`, `Surname`, `Email`, `Password`, `Date of Birth`, `Gender`, `Status`, `Phone`, `Address` FROM `counsellor` WHERE `Firstname`='$fname' AND `Lastname`='$lname' AND `Surname`='$sname' AND `Email`='$email' AND `Password`='$pwd' AND `Date of Birth`='$dob' AND `Gender`='$gender' AND `Status`='$status' AND `Phone`='$phone' AND `Address`='$address'";
            $query_run = mysqli_query($con, $query);
            $sql = "SELECT `Email` From `counsellor` WHERE `Email`='$email'";
            $result =mysqli_query($con, $sql);
            if(mysqli_num_rows($result) > 0){
                $message = "There is already a user with that email!";
            }else{
                $query = "INSERT INTO `counsellor` VALUES ('','".($fname)."','".($lname)."','".($sname)."','".($email)."','".($pwd)."','".($dob)."','".($gender)."','".($status)."','".($phone)."','".($address)."')";
...
<a href="../records/counsellor.php" class="nav-link px-0 align-middle">
