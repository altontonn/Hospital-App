...
<?php
    ...
    if(isset($_POST['fname']) && isset($_POST['lname']) && isset($_POST['sname']) && isset($_POST['email']) && isset($_POST['pwd']) && isset($_POST['dob']) && isset($_POST['gender']) && isset($_POST['status']) && isset($_POST['phone']) && isset($_POST['address'])){
        ...
        if(!empty($fname) && !empty($lname) && !empty($sname) && !empty($email) && !empty($pwd) && !empty($dob) && !empty($gender) && !empty($status) && !empty($phone) && !empty($address)){
            $query = "SELECT `Firstname`, `Lastname`, `Surname`, `Email`, `Password`, `Date of Birth`, `Gender`, `Status`, `Phone`, `Address` FROM `nurse` WHERE `Firstname`='$fname' AND `Lastname`='$lname' AND `Surname`='$sname' AND `Email`='$email' AND `Password`='$pwd' AND `Date of Birth`='$dob' AND `Gender`='$gender' AND `Status`='$status' AND `Phone`='$phone' AND `Address`='$address'";
            $query_run = mysqli_query($con, $query);
            $sql = "SELECT `Email` From `nurse` WHERE `Email`='$email'";
            $result =mysqli_query($con, $sql);
            if(mysqli_num_rows($result) > 0){
                $message = "There is already a user with that email!";
            }else{
                $query = "INSERT INTO `nurse` VALUES ('','".($fname)."','".($lname)."','".($sname)."','".($email)."','".($pwd)."','".($dob)."','".($gender)."','".($status)."','".($phone)."','".($address)."')";
    ...
    <div class="message text-success bg-light text-center fs-5" ><?php if($success!="") { echo $success; } ?></div>
        <div class="row">
            <div class="col-lg-6">
                ...
                <div class="form-group p-3">
                    <label for="pwd">Nurse Password:</label>
                    <input type="password" class="form-control" id="pwd" placeholder="Enter password" name="pwd">
                </div>
            </div>
            <div class="col-lg-6">
                <div class="form-group p-3">
                    <label for="dob">Nurse Date of Birth:</label>
                    <input type="date" class="form-control" id="DOB" placeholder="Enter Date of Birth" name="dob">
                </div>
                <div class="form-group p-3">
                    <label for="Gender">Nurse Gender:</label><br>
                    <input type="radio" name="gender" value="Male"> Male
                    <input type="radio" name="gender" value="Female"> Female
                </div>
                <div class="form-group p-3">
                    <label for="status">Nurse Status:</label>
                    <input type="status" class="form-control" id="status" placeholder="Enter status" name="status">
                </div>
                <div class="form-group p-3">
                    <label for="phone">Nurse Phone:</label>
                    <input type="phone" class="form-control" id="phone" placeholder="Enter Phone" name="phone">
                </div>
                <div class="form-group p-3">
                    <label for="address">Nurse Address:</label>
...
<a href="../records/nurse.php" class="nav-link px-0 align-middle">
